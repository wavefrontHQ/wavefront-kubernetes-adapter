    ...
    stages {
        ...
        stage('Create Bump Version PR') {
            ...
            steps {
                ...
                script {
                    env.NEW_VERSION_NUMBER = sh(script: 'cat release/VERSION', returnStdout: true).trim()
                }
                sh 'echo "New version number: ${NEW_VERSION_NUMBER}"'
                sh 'git branch -D bump-version-$(cat release/VERSION) &>/dev/null || true'
